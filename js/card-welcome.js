...
window.WELCOME_CONFIG = {
    API_KEY: 'oZqyhubEHUfiZvqUjowAiWtbbf', // API密钥
    BLOG_LOCATION: {
        lng: 113.18, // 博主经度
        lat: 22.60   // 博主纬度
    },
...
const fetchIpInfo = async (container) => {
    try {
        ...
        // 请求API
        const response = await fetch(`https://api.nsmao.net/api/ipip/query?key=oZqyhubEHUfiZvqUjowAiWtbbf`);
        if (!response.ok) {
