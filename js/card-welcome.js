...
function showWelcome(ipLocation) {
    ...
    let dist = getDistance(113.392, 22.574, ipLocation.data.lng, ipLocation.data.lat);
    ...
    else if (date.getHours() >= 13 && date.getHours() < 19) timeChange = "<span>🕞 下午好，饮茶先啦！</span>";
    else if (date.getHours() >= 19 && date.getHours() < 24) timeChange = "<span>🌙 晚上好，不要熬夜啊，会肾虚的😭</span>";
    ...
    if (welcomeInfoElement) {
        welcomeInfoElement.innerHTML =
            `欢迎来自<br><b><span style="color: var(--default-bg-color)">${pos}</span></b><br> 的访客，你好呀！😝<br>${posdesc}🍂<br>你目前距博主约 <b><span style="color: var(--default-bg-color)">${dist}</span></b> 公里！<br>你的网络IP为：<b><span class="ip-address" style="font-size: 15px;">${ip}</span></b><br>${timeChange} <br>`;
    } else {
