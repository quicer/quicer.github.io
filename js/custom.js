...
async function fetchHistoryData() {
    ...
    const historyDataUrl = `https://api.nsmao.net/api/history/query?key=TJtSoMT2biwk2PBxiSLMx03VnH`; //申请：https://api.nsmao.net
...
return;
 ...
 let timeChange;
 let date = new Date();
 if (date.getHours() >= 5 && date.getHours() < 11) timeChange = "<span>🌤️ 早上好！起得真早啊~</span>";
 else if (date.getHours() >= 11 && date.getHours() < 13) timeChange = "<span>☀️ 中午好，记得午休喔~</span>";
 else if (date.getHours() >= 13 && date.getHours() < 17) timeChange = "<span>🕞 下午好，吃下午茶~</span>";
 else if (date.getHours() >= 17 && date.getHours() < 19) timeChange = "<span>🚶‍♂️ 即将下班，记得按时吃饭~</span>";
 else if (date.getHours() >= 19 && date.getHours() < 24) timeChange = "<span>🌙 晚上好，早点睡，熬夜伤身</span>";
 ...
 if (welcomeInfoElement) {
     welcomeInfoElement.innerHTML =
         `你好！欢迎来自 <b><span style="color: var(--efu-main)">${pos}</span></b> 的访客<br>当前位置距quicer约 <b><span style="color: var(--efu-main)">${dist.toFixed(2)}</span></b> 公里！<br>${timeChange}<br>Tip：<b><span style="font-size: 15px;">${posdesc}</span></b>`;
 }
}
