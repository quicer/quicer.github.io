...
(function () {
  ...
  // ---- 中间 nav 下拉菜单：position: fixed + JS 定位 ----
  // .menus_item_child 仍嵌套在 .menus_item 内（DOM 不动），但改成 position: fixed。
  // 这样它的 backdrop-filter 直接参考 viewport 真实页面背景，不再受 .menus_items
  // 的 backdrop-filter / backdrop root 限制，真正做出强毛玻璃。
  // 显示/隐藏由 JS 切换 .show 类，位置由 JS 根据触发项 viewport 坐标实时计算。
  function initNavDropdowns() {
    var items = document.querySelectorAll('#menus .menus_item');
    var activeItems = [];
    items.forEach(function (item) {
      if (item.querySelector('.menus_item_child')) activeItems.push(item);
    });
    ...
    function getDrop(item) {
      return item.querySelector('.menus_item_child');
    }

    function positionDrop(drop, item) {
      var rect = item.getBoundingClientRect();
      // 先显示出来才能拿到 offsetWidth；如果还没显示，先让浏览器布局一帧。
      var dropWidth = drop.offsetWidth || 0;
      var top = rect.bottom + 8;
      var left = rect.left + rect.width / 2 - dropWidth / 2;
      drop.style.top = top + 'px';
      drop.style.left = left + 'px';
    }

    function show(item) {
      var drop = getDrop(item);
      if (!drop) return;
    ...
    activeItems.forEach(function (item) {
      ...
      item.addEventListener('mouseleave', function () { hide(item); });
      drop.addEventListener('mouseenter', function () { show(item); });
      drop.addEventListener('mouseleave', function () { hide(item); });
    });
    ...
    if (window.__navDropdownUpdateBound !== true) {
      window.__navDropdownUpdateBound = true;
      window.addEventListener('resize', updateVisible);
      window.addEventListener('scroll', updateVisible, true);
    }
  }
