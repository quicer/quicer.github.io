...
const loadFeatureModules = async () => {
  ...
  if (features.weather) {
    requests.push(import("./weather.js?v=6").then((module) => {
  ...
  const select = (index) => {
    ...
    if (titleLink.animate) {
      ...
      titleTag?.animate(
        ...
        { duration: 480, delay: 90, easing: "cubic-bezier(0.34, 1.56, 0.64, 1)" }
      );
    }
...
const actions = {
  ...
  initbbtalk() {
    ...
    if (!bberTalkElement || !container || container.swiper) return;
    if (typeof Swiper === "undefined") {
      setTimeout(() => Solitude.initbbtalk(), 120);
      return;
    }
...
class toc {
  ...
  static active(toc) {
    ...
    const moveIndicator = (activeEl) => {
      if (!activeEl || !$tocContent) return;
      let indicator = $tocContent.querySelector(".toc-indicator");
      if (!indicator) {
      ...
      const textSpan = activeEl.querySelector(".toc-text");
      const target = textSpan || activeEl;
      const contentRect = $tocContent.getBoundingClientRect();
      const targetRect = target.getBoundingClientRect();
      const paddingX = 12;
      const paddingY = 4;

      indicator.style.width = `${targetRect.width + paddingX * 2}px`;
      indicator.style.height = `${targetRect.height + paddingY * 2}px`;
      indicator.style.left = `${targetRect.left - contentRect.left + $tocContent.scrollLeft - paddingX}px`;
      indicator.style.top = `${targetRect.top - contentRect.top + $tocContent.scrollTop - paddingY}px`;
...
const bindGlowCard = (selector, root = document) => {
...
const initAboutCardGlow = () => {
  ...
  bindGlowCard("#about-page .author-content-item");
};

const initEssayCardGlow = () => {
  if (!document.getElementById("bber")) return;
  bindGlowCard("#bber .timeline ul li.item");
};

const initRecentCommentCardGlow = () => {
  const container = document.querySelector("#page .console_recentcomments");
  if (!container) return;

  bindGlowCard(".comment-card", container);
