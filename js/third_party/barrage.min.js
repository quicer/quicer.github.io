...
class Barrage {
    ...
    initBarrage() {
        // 默认显示评论弹幕；仅在用户明确关闭（false）时隐藏
        const saved = utils.saveToLocal.get('commentBarrageSwitch');
        const isBarrageEnabled = saved !== false;
...
function initializeCommentBarrage(comments) {
    const dom = document.querySelector('.comment-barrage');
    // 只在文章页（post）显示热评弹幕，其他页面清理并隐藏
    const isPostPage = document.body.classList.contains('post') || !!document.getElementById('post-comment');
    if (!isPostPage) {
        if (window.currentBarrage) window.currentBarrage.destroy();
